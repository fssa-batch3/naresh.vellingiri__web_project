...
fundraiser_list.find((obj) => {
  if (obj.emerging_player_id == emer_id) {
    ...
    containPlayersCertificateImageDiv.classList.add(
      "contain_players_certificate_image"
    );

    const cert_arr = obj.certificate_arr;
...
//  leftContainer.appendChild +=video_div
    ...
    const number = Number(obj.minimum_amount);
    const total_raised_value = Number(obj.total_raised_value);
    ...
    amtHeading.innerHTML = `<b style="color:black";>${obj.total_raised_value.toLocaleString(
      "en-IN",
      {
...
function amount_calc() {
  ...
  const total_raised_value = Number(get_obj.total_raised_value);

  const divided_value = (total_raised_value / number) * 100;
...
fundraiser_list.find((obj) => {
  if (obj.emerging_player_id == emer_id) {
    const find_donar = obj.donar_list;

    if (find_donar != undefined) {
      document.getElementById(
        "suporters1"
      ).innerHTML = `${find_donar.length} Supporters`;
    } else {

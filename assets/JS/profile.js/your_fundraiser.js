const fundraiser = JSON.parse(localStorage.getItem("fundraiser_list"));
...
const fundraiser_list = fundraiser.filter((obj) => {
...
const detail_fund_raisers_flex = document.getElementById(
  "detail_fund_raisers_flex"
);

// create the main container element

function show_list() {
  const fundraiser_list_one = fundraiser.filter((obj) => {
    if (login_status[0].user_id === obj.raiser_user_id) {
      return "1";
    }
    return false;
  ...
  fundraiser_list_one.forEach((item, index) => {
    ...
    const number = Number(item.total_raised_value);
    const minimum_amount = Number(item.minimum_amount);

    const dived_value = (number / minimum_amount) * 100;
...
${minimum_amount.toLocaleString("en-IN", {
  ...
  currency: "INR",
    ...
    const total_days_left = days_calculation(item.days_left);
    ...
    btn.setAttribute(
      "onclick",
      `editemerginplayer(${item.emerging_player_id})`
    );
    btn.innerText = "Edit";
    ...
    delete_btn.setAttribute(
      "onclick",
      `deleteemerplayer(${item.emerging_player_id})`
    );
...
const form_creation_fundraiser1 = document.getElementById(
  "form_creation_fundraiser1"
);
...
const form_creation_fundraiser = document.getElementById(
  "form_creation_fundraiser"
);
const form_creation_fundraiser_specific_details = document.getElementById(
  "form_creation_fundraiser_specific_details"
);
...
const cert_form = document.getElementById("cer_form");
const certificate_image_url = document.getElementById("certificate_image_url");
const cerficate_number = document.getElementById("certificate_number");

const cer_append_div = document.querySelector(".display_cer_list");
...
function displaycer() {
  ...
  const get_cer = get_copy.certificate_arr;
...
cert_form.addEventListener("submit", (e) => {
  const get_copy_data = JSON.parse(localStorage.getItem("copy_player"));

  const get_cer = get_copy_data.certificate_arr;

  e.preventDefault();

  const cer_img_url = certificate_image_url.value.trim();
  const cer_num = cerficate_number.value.trim();
...
raise_fund.addEventListener("submit", (e) => {
  ...
  const get_amount = minimum_amount.value;
  const get_days = days_left.value;
  const get_bio = show_bio.value;
...
const cross_mark_1 = document.getElementById("cross_mark_1");
